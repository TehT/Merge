...
## proficiency-rank coverage vs. the check's requirements sets a success
...
## missed. This is MissionCheck's default strategy when none is assigned,
...
func resolve(check: MissionCheck, squad: Array[AgentData]) -> MissionResolutionResult:
	var computed := MissionResolver.compute_team_suitability_explained(
			check.get_proficiency_requirements(), squad, check.tags)
